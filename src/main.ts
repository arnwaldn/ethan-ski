/**
 * ETHAN SUPERSTARS DU SKI - Professional Level Graphics
...
// ==================== PROFESSIONAL SKI RACING CONFIG ====================
const GAME = {
  // Course - Inspired by real professional alpine racing tracks
  COURSE_LENGTH: 8000,
  COURSE_WIDTH: 60,
...
function createFinishArena() {
  ...
  // Spectator jacket colors - winter sports theme
  const jacketColors = [
...
function createSkier() {
  ...
  // Tenue du skieur : couleurs du personnage
  ...
  // Head (face visible above neck warmer) - Teint du personnage
  const skinMat = new THREE.MeshStandardMaterial({
    color: 0xF5D5C8,  // Teint du personnage
    roughness: 0.9,
    metalness: 0.0
  });
  const head = new THREE.Mesh(
...
function updateUI() {
  ...
  if (state.penalties > 0) {
    ...
    document.getElementById('penalties-value')!.textContent = '+0.0s';
    document.getElementById('penalties-box')!.classList.add('hidden');
  }
...
// ==================== INIT ====================
async function init() {
  console.log('🎿 Initializing Ethan Superstars du Ski with Professional Graphics...');
  ...
  console.log('🏔️ Ethan Superstars du Ski ready!');
